...
def add_arguments(parser):
    group = parser.add_argument_group(title='OLMo-2 HF loader.')
...
def load_args_from_checkpoint(args):
    ...
    args.use_attention_kv_norm = True
...
def set_attn_state(args, layer, hf_layer):
    ...
    # olmo attn norms
    attn.q_norm.weight.data.copy_(hf_attn.q_norm.weight)
    attn.k_norm.weight.data.copy_(hf_attn.k_norm.weight)
...
def set_layer_state(args, model, hf_model, layer_idx):
    ...
    # change input_layernorm to post_feedforward_layernorm
    layer.post_feedforward_norm.weight.data.copy_(hf_layer.post_feedforward_layernorm.weight)
...
def load_checkpoint_to_model(args):
    ...
    hf_model = Olmo2ForCausalLM.from_pretrained(args.load)#, device_map="cpu")
...
def _load_checkpoint(queue, args):
    ...
    assert args.model_type == 'GPT', 'Olmo-2 is a GPT model.'
    ...
    md.use_attention_kv_norm = True
    md.post_feedforward_norm = True
    md.apply_layernorms_before_residuals = True
    ...
    for layer_num in range(margs.num_layers):
        ...
        message["post ff norm weight"] = layer.post_feedforward_norm.weight.data
        ...
        if md.use_attention_kv_norm:
            q_norm_weight.append(layer.self_attention.q_norm.weight.data)
            k_norm_weight.append(layer.self_attention.k_norm.weight.data)
        ...
        if md.use_attention_kv_norm:
            message['q_norm weight'] = torch.cat(q_norm_weight, dim=0)
            message['k_norm weight'] = torch.cat(k_norm_weight, dim=0)
